...
### Conventions of the signed-in pages (post UX review, September 2026)

Three rules came out of a screen-by-screen review and every page now follows them; a new page must too:

- **One header.** Every page starts with `<PageHeader size="page" eyebrow="…" title="…" lead="…" />` from `components/editorial.tsx` — eyebrow « Espace professeur », « Espace élève », « Espace connecté » (Messages, Mon compte) or « Administration ». Before that, eleven pages had three different headers and three had none (the page opened on tabs). Sheets (student sheet, teacher dossier) keep their avatar but use `Eyebrow` + `PageTitle size="page"` in the same rhythm.
- **One width.** `app/dashboard/layout.tsx` centres everything in a `max-w-5xl` container; pages must **not** add `mx-auto max-w-*` (the content used to jump sideways between 3xl, 4xl and 5xl pages). The agenda simply takes the full width.
- **One browser title per page.** `export const metadata = { title }` on every server page; the root layout applies the `%s | SiNote` template. Every signed-in tab used to carry the landing title.

Shared helpers born from the same review, to use instead of re-inventing: `formatPrice` (`lib/format/price.ts` — « 45 € », « 42,50 €», never `toFixed(2)`), `instrumentInProse` (`lib/instruments/prose.ts` — « cours de guitare électrique », keeps sigles like MAO/DJ; `lessonTitle` already uses it), `ConfirmDialog` (`components/ui/confirm-dialog.tsx` — every irreversible action: decline, cancel, no-show, student cancel; `cancel`/`decline` carry an optional `reason` that the route stores as `cancellationReason`), `PageSkeleton` (`components/page-skeleton.tsx`, rendered by the three `loading.tsx`), `summarizeOpenings` (`lib/teacher/openings-summary.ts`, the « Disponible : lun. matin · jeu. soir » line on the public profile).

Other decisions from that review worth not undoing: sign-up asks for **prénom (required) and nom** and Better Auth writes them through `user.additionalFields` + a `databaseHooks.user.create.before` that recomposes `name` (the email local part is never a display name again); the sign-in and account screens are real `<form>`s (Enter submits); the home search treats « visio / en ligne / à distance » typed as a city as `mode=online` instead of a `ville` filter (the « En visio » checkbox was tried and removed); the landing répertoire lists the **whole catalogue** (taught instruments in bold) so `/cours/*` stays reachable when no teacher is visible; `SiteFooter` carries the legal links and the three legal pages (`/mentions-legales`, `/cgu`, `/confidentialite`) are `noindex` with `[À compléter : …]` placeholders until the founder fills them; the account menu (`UserNav`) no longer duplicates the sidebar; `FicheTabs` counts are muted text, only `unread` gets the primary pill; the student's lesson rows link to the dossier's report instead of embedding it with a composer; the agenda writes « Fermé » once per fully-closed day and the month view greys days with no opening (same `dayOpenings` as the engine); the booking widget shows the week label and the next available slot when a week is empty.

### Refonte « Le Pupitre » (17 septembre 2026)

The signed-in area was redrawn from mockups, keeping the tokens, the fonts and the **dark Prussian-blue sidebar** (the founder's explicit choice — the mockups had a light rail, the code does not). What changed is the structure, and these are the load-bearing decisions:

- **Six sidebar entries for a teacher, three for a student**, down from eleven and six (`components/dashboard-sidebar.tsx`). Teacher: Aujourd'hui (`/dashboard`), Demandes, Agenda, Élèves, then a « Mon activité » group with Ma fiche and Activité. Student: Mes cours (`/dashboard`), Mes profs (`/dashboard/dossiers`), Mon profil (`/dashboard/cours/profil`). Messages and « Trouver un prof » sit under a hairline for both. What left the menu did **not** leave the app: Disponibilités is the « Horaires » tab of the agenda (`AgendaViewSwitch`, still at `/dashboard/prof/disponibilites`); Avis and Abonnement are tabs of Ma fiche (`components/ma-fiche-tabs.tsx`, same URLs); Comptes rendus is linked from Élèves and from the « À traiter » list. **« Ma fiche » carries an amber dot whenever `visibilityBlocker()` returns something** — with Avis and Abonnement out of the menu, a lapsed subscription would otherwise hide the profile in silence. The dot is the `attention` prop of the sidebar, computed in `app/dashboard/layout.tsx` from the same rule as the home-page notice.
- **The home page is per role and lives in `components/home/`.** `app/dashboard/page.tsx` only reads the role and renders `TeacherHome` (« Aujourd'hui » : the day as a timeline, then « À traiter », the profile's status and unread messages on the right) or `StudentHome` (« Mes cours » : one card for the next lesson, « À venir », « Cours passés », a mini-month on the right). The old « Tableau de bord » with four tiles is gone.
- **Old student routes redirect**: `/dashboard/cours` (« Mes réservations ») and `/dashboard/agenda` (the weekly grid, almost always empty for a student with two lessons a month) both go to `/dashboard`. The student agenda component was deleted; `components/student-month.tsx` (`?mois=AAAA-MM` in the URL, day keys in the student's zone) replaces it. `MarkCoursSeen` moved to `components/mark-cours-seen.tsx` with its action in `lib/student/mark-cours-seen.ts`.
- **The next free slot is read before the profile is opened.** `lib/teacher/next-slots.ts` (`getNextSlotsForTeachers`) runs the slot engine for a whole page of teachers in three queries and feeds the landing rows, the search results, the /cours pages, the profile's « Prochain créneau » block (server-rendered, so it is in the HTML) and the student dossier's « Réserver à nouveau ». Same inputs as the public availability route, so what is promised in a list is what the widget offers. `formatSlotShort` lives in `lib/teacher/slot-label.ts`, **without Prisma**, because the booking widget (a client component) writes the same phrases.
- **The landing hero is the pre-redesign one, on the founder's request**: the two-field search bar (`HeroSearch`, instrument + ville, placeholder « … ou en visio », no checkbox since 13 September 2026) and the animated medallion. Only the sections below it changed (teacher rows with next slot, répertoire by family, three steps, teacher band). Do not swap the hero for the « sentence » search of the mockups.
- **Search filters are a left column** (instrument list with per-instrument counts of *visible* teachers, city, a real `<input type=range>` for the price ceiling, trial lesson) and a « Tous / En visio / En présentiel » segment, because `mode=online` *restricts* — an « inclure la visio » switch would have lied about its effect. Under `lg` the column folds behind a « Filtres » button. Results carry family-coloured instrument chips (`components/instrument-chip.tsx`) and up to three slot chips; the row is no longer one link, since an `<a>` cannot contain another.
- **The booking widget shows a 7-day strip** (slot count under each day) and lists the selected day's slots by period, on the hour by default with a « + quarts d'heure » disclosure when the granularity is finer. The strip renders after mount only: `startOfWeek` reads the clock, and server and browser can land on different Mondays.
- **The agenda opens the selected lesson in a side inspector** (`LessonInspector` in `components/teacher-agenda.tsx`), 320px on the right from `lg`, a bottom sheet below. Actions are still `checkTransition`'s. Status labels and tints come from one place, `components/lesson-status.tsx` — three screens used to copy them, with two spellings for `NO_SHOW` (now « Absent » everywhere, legend included).
- **The student profile shows what the teacher will receive** (`components/student-profile-preview.tsx`, fed by the form's live state), the level is a segmented control (four levels — `PROFESSIONAL` exists in the enum), the guardian block appears only when the birth date implies a minor, and `preferredGenres` finally has a UI (`lib/student/genres.ts`; the route already accepted it).
- « Le prof répond en général sous 24 h » is only written when it has been measured: `lib/student/response-time.ts` takes the median of at least three answered requests, rounded up to a bucket, else says nothing.

Three gabarit decisions taken during the fidelity pass of 20 September 2026: **public pages are 1312px wide** (`max-w-[82rem]`, `px-4 sm:px-8` on header, footer, landing, search, profile, /cours, /enseigner — the mockups are drawn at 1440 with 64px gutters, and at 1024px the search results had 330px left for the teacher's identity); **`PageTitle size="page"` is 52px** on `sm+` (the mockups' in-app title size; the sheets add their own 56px); **the agenda page widens the signed-in container** with the `page-wide` class (a `main > div:has(> .page-wide)` rule in `globals.css`, because the layout is a Server Component that cannot read the route). The `h1, h2, h3` Cormorant rule is written `:where(h1, h2, h3)` (zero specificity), so a `font-sans` utility can override it — before, an inline style was the only way out; `@layer base` was tried first and Next's dev CSS pipeline emits the rule outside any layer. One deliberate divergence from the mockups stays: agenda blocks keep confirmed = blue / terminé = green (the mockups have them the other way round), for the reason recorded in the agenda section — a green `Terminé` is the only colour that collides with nothing on that grid.

**Two-column pages declare their single column too**: `grid grid-cols-[minmax(0,1fr)] … lg:grid-cols-[minmax(0,1fr)_320px]`. Without the first class the implicit track below `lg` is `auto`, and it grows to the widest child (the mini-month, a truncated row), so the whole page scrolled sideways on a phone. Same trap on the landing: a `shrink-0` note inside a `SectionTitle` widened the page; it is hidden under `md`.

A known, **pre-existing** dev-only warning: the Radix dropdown in `UserNav` logs a hydration mismatch on its generated `id` in the signed-in area. It predates the redesign and does not affect the rendered page.
...
`DashboardSidebar` is a Client Component for one reason — marking the current entry needs `usePathname`. Its item lists live **in the client component**, not in the layout: Lucide icons are components, and a component cannot cross the server→client boundary ("Only plain objects can be passed to Client Components"). The layout passes numbers (`badges`) and strings (`attention`).
...
`/dashboard` renders the role's home (`components/home/`). Adding a role-specific area means adding it to the sidebar's item list too, or it stays URL-only.
